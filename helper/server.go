...
func reqHandler(conn net.Conn) {
	...
	req, err := read(conn, DELIM) // 从连接中读取 请求字节数据
	if err != nil {
		errMsg = fmt.Sprintf("Server: Req Read Error: %s", err)
	} else {
		var sreq ServerReq
		err := json.Unmarshal(req, &sreq) // 将请求字节请求数据 转为 请求处理方法可识别的结构体ServerReq，就像Springboot会将http协议中的请求数组 转为 接口方法请求参数实体类
		if err != nil {
	...
	_, err = write(conn, bytes, DELIM) // 将响应写入到连接中
	if err != nil {
...
func (server *TCPServer) Listen(addr string) error {
	...
	go func() { //此协程用来接收连接请求，
		for {
			...
			conn, err := server.listener.Accept() // 接收连接请求
			if err != nil {
				if atomic.LoadUint32(&server.active) == 1 {
			...
			go reqHandler(conn) // 然后为每个连接，开启一个协程，用于处理请求
		}
	}()
	return nil
}
